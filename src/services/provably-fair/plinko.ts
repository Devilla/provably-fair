...
export function plinko(server_seed: string, client_seed: string, nonce: string, rows: number, risk: string): number {
    let nums: number[] = [];
    let payoutIndex = final_payout(risk, rows).length;
    console.log(payoutIndex);
    bytes_to_num_array(bytes(`${server_seed}${client_seed}${nonce}`)).forEach((value, index) => {
